...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $religion = filter_var($_POST['idNumber'] ?? '', FILTER_SANITIZE_SPECIAL_CHARS);
    $civilStatus = filter_var($_POST['bloodType'] ?? '', FILTER_SANITIZE_SPECIAL_CHARS);
    ...
    if (!preg_match('/^\d+(\.\d+)?$/', $height)) {
        $submissionError = "Height must be a valid number (e.g., 170 or 170.5).";
    }
    if (!preg_match('/^\+?[\d\s-]{10,}$/', $phone)) {
        $submissionError = "Please enter a valid phone number.";
    }
    if (!preg_match('/^\+?[\d\s-]{10,}$/', $emergencyPhone)) {
        $submissionError = "Please enter a valid emergency phone number.";
    }
    if (!preg_match('/^\+?[\d\s-]{10,}$/', $guardianPhone)) {
        $submissionError = "Please enter a valid guardian phone number.";
    }

    // Handle photo upload (not inserted into DB, but saved to filesystem)
    $photoPath = '';
    if (isset($_FILES['studentPhoto']) && $_FILES['studentPhoto']['error'] == 0) {
        ...
        if (in_array(strtolower($ext), $allowed)) {
            ...
            if (move_uploaded_file($_FILES['studentPhoto']['tmp_name'], $uploadPath)) {
                ...
                $submissionError = "Error uploading photo.";
            }
        } else {
            $submissionError = "Invalid photo format. Only JPG, JPEG, or PNG allowed.";
        }
    }
    ...
    if (!isset($submissionError)) {
        $sql = "INSERT INTO student_applications (
            ...
            guardian_occupation, guardian_employer, guardian_email, 
        ...
        if ($stmt) {
            $stmt->bind_param(
                ...
                $campus, $department, $guardianFirstName, $guardianMiddleName,
                $guardianLastName, $guardianRelationship, $guardianDob,
                $guardianOccupation, $guardianEmployer, $guardianEmail,
            ...
            if ($stmt->execute()) {
                ...
                $submissionError = "Error submitting application: " . $stmt->error;
            }
            $stmt->close();
        } else {
            $submissionError = "Error preparing statement: " . $conn->error;
            // Debug output
            echo "SQL: $sql<br>";
            echo "Parameters: " . print_r([
                $firstName, $lastName, $middleName, $gender, $age, $height, $dob,
                $nationality, $religion, $civilStatus, $email, $phone, $address,
                $emergencyName, $emergencyRelation, $emergencyPhone, $emergencyEmail,
                $campus, $department, $guardianFirstName, $guardianMiddleName,
        ...
        <nav class="navigation">
            ...
            <div class="nav-btn">
                <button class="btn-logout">
                    <i class="fas fa-sign-out-alt"></i><a href="login.php">Logout</a>
                </button>
            </div>
        </nav>
    </header>
        ...
        <form id="studentApplicationForm" class="form-container" method="POST" enctype="multipart/form-data">
            <div class="form-section">
                <h2 class="section-title">
                    <i class="fas fa-user-graduate"></i> Personal Informations
                </h2>
    ...
    <div class="popup-overlay" id="submissionPopup">
        <div class="popup-content <?php echo isset($submissionError) ? 'error-popup' : ''; ?>">
            ...
            <div class="popup-buttons">
                ...
                <button class="popup-btn popup-btn-secondary" onclick="redirectToDashboard()">Go to Dashboard</button>
            </div>
        </div>
    </div>
